...
class Jugador:
    ...
    def muestra_carta(self) -> CartaPoker:
        return self.__cartas.pop(0)

    def toma_cartas(self, carta:CartaPoker) -> None:
...
class BatallaCartas:
    ...
    def __repartir_mazo(self, jug1: Jugador, jug2: Jugador) -> None:
        ...
        for a in range(0,26):
            ...
            jug1.toma_cartas(carta)
            carta= mazo.sacar_carta()
            jug2.toma_cartas(carta)

    def __compara_cartas(self, jug1: Jugador, jug2: Jugador, pozo: MazoPoker) -> MazoPoker:
        carta_jug1= jug1.muestra_carta()
        print (f"{jug1.get_nombre()} juega {carta_jug1}")
        carta_jug2= jug2.muestra_carta()
        print (f"{jug2.get_nombre()} juega {carta_jug2}")

        if carta_jug1.get_numero()>carta_jug2.get_numero():
            jug1.toma_cartas(carta_jug1)
            jug1.toma_cartas(carta_jug2)
            print (f"Gana {jug1.get_nombre()}")
            if pozo.__len__()>0:
                jug1.toma_pozo(pozo)

        elif carta_jug1.get_numero()<carta_jug2.get_numero():
            jug2.toma_cartas(carta_jug2)
            jug2.toma_cartas(carta_jug1)
    ...
    def jugar(self):
        ...
        while len(self.__jugador1)>0 and len(self.__jugador2)>0 and mano<rondas:
            print (f"Ronda {mano}")
